...
class TranscriptManager:
    ...
    def _timestamps(self) -> bool:
        """When True (default), request ``verbose_json`` from the API so that
        segment-level timestamps are available, and write the JSON output as
        ``[{"start": ..., "end": ..., "text": ...}]`` instead of the raw
        API payload.
        """
        v = self._cfg().get("timestamps", True)
        if v is None:
            return True
        return bool(v)
    ...
    async def _write_outputs(
        ...
        if "json" in formats:
            segments = payload.get("segments") if self._timestamps() else None
            ...
            async with aiofiles.open(json_path, "w", encoding="utf-8") as f:
                await f.write(json.dumps(json_output, ensure_ascii=False, indent=2))
    ...
    async def _call_openai_transcription(
        ...
        *,
        ...
        form.add_field("response_format", "verbose_json" if self._timestamps() else "json")
        if language_hint:
